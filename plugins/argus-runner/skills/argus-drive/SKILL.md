...
## -1. 先确定 argus 本体怎么调（插件环境必读）
插件目录里没有 argus 的 Python 包，本 skill 全程靠 argus 本体的 CLI。开工前先定好前缀，之后**下文所有 `argus …` 都用它替换**：

```bash
# 有 ARGUS_HOME（clone 了仓库，未 pip install —— 最常见）：
A="env PYTHONPATH=$ARGUS_HOME python3 -m argus.cli"
# 已 pip install argus：
A="python3 -m argus.cli"
$A devices --json          # 验证一下能跑通；不通先跑 /argus-doctor
```
用例目录默认在 `$ARGUS_HOME/tests/<target>/`，报告也落那里。依赖/LLM key 缺失时**不要硬跑**，先 `/argus-doctor`。
