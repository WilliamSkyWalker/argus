...
## 2b. 挂了 probe 的 step：不许自己看屏判，去调插件
step 下面一行是 `# argus-probe: <name> k=v` 的（埋点/后端落库这类**看不见**的断言）：**别用视觉判**，跑
`argus probes check <name> <k=v...> --target <target> --wait`（退出码 0=pass），把 stdout 的 evidence 原样写进 journal / 报告并打 `[probe:<name>]` 前缀。
- 插件返回 inconclusive = 数据还没上报（分钟级 flush 延迟），`--wait` 会自己轮询到预算耗尽 —— **别因为查了一次是 0 就判 fail**（这么干过，造出过假 bug）
- 没配 `.argus/probes.json` / 插件名没注册 → 该 step 判 **fail**，别静默跳过（一条没验证的断言当 pass 就是「不可视断言禁 PASS」铁律禁的事）
- 用户明确说「跳过埋点」时：标 **skip 不标 pass**，并在报告 reason 里点名哪几步没验证（对齐 `argus run --skip-probes`）；说「只跑埋点」时只挑含 directive 的 case 跑，但**这些 case 的 UI 步照跑**（埋点得先被操作触发）
- 细节见 `docs/probes.md`
