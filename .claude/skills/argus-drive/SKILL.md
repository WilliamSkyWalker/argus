...
## 平台层：优先用 argus MCP device tools（不要手搓 adb/u2）

argus MCP server 已把平台层原语暴露成 tool（复用 `argus.platforms.android.AndroidPlatform`，
持久进程缓存一个 u2 连接，跨 call 复用 —— 比每次 spawn `python3`/`adb` 稳，
不会反复弹 Android IME 选择器）：

| MCP tool | 作用 | 关键点 |
|---|---|---|
| `device_screenshot(serial, out_path?)` | 截屏落盘 | 返回 `path` + `screen_size` + **`scale`**（≠1 说明 screencap 被缩放，坐标要换算——点不中先看这里） |
| `device_tap(x, y, serial)` | 点击 | 设备像素坐标；含可选 snap-to-clickable |
| `device_input(text, serial)` | 写文字 | u2 ACTION_SET_TEXT 绕 IME，CJK 安全；需先 tap 聚焦输入框；**不发送** |
| `device_type_send(text, input_x, input_y, send_x, send_y, wait_s, serial, out_path?)` | 填字段+提交+等+截屏 一步到位 | 跑很多轮「填+提交+观察」（如 chat 压测）省往返；返回同 `device_screenshot` |
| `device_swipe(x1,y1,x2,y2,duration_ms,serial)` | 滑动 | 惯性滚动调大 duration |
| `device_key(key, serial)` | 按键 | enter/back/home/…；⚠️ 部分 Flutter App 里 back 会退出 App |
| `device_ui_tree(serial, max_chars?)` | dump 树 | Flutter App 拿不到可用树 → 照截图视觉定位 |
| `device_launch(package, activity?, serial, force_stop?)` | 拉起/relaunch App | |

**用法**：`device_screenshot` → Read 图 → 视觉定位 → `device_tap`/`device_input`/`device_swipe`。
多设备并发时每个 call 显式传 `serial`。**若这些 MCP tool 不可用**（server 未挂 / 旧连接），
才降级到下面 §2 的裸 `adb` 命令。
...
**优先路径（MCP）**：`device_screenshot` 直接返回 `scale`（截图px/屏幕px），
不用自己 struct 解 PNG 头标定 —— `scale==1` 照截图坐标直接点，`scale!=1` 把截图
坐标 ×(1/scale) 换算成设备坐标再传 `device_tap`。单步就是：
`device_screenshot(serial,out_path)` → Read 图 → **视觉定位** → `device_tap(x,y,serial)` → 回到截图。

**降级路径（裸 adb，仅 MCP 不可用时）**——先标定分辨率：
...
adb shell screencap -p /sdcard/argus.png && adb pull /sdcard/argus.png /tmp/argus-drive/shots/<TC>-<step>.png
# → Read 截图 → 视觉定位 → 按比例换算 → adb shell input tap X Y → 回到截图
```

⚠️ Flutter App 的输入框：裸 `adb shell input text` 对 CJK 无效，
必须走 `device_input`（u2 set_text）；用裸 adb 时中文打不进去。
...
1. **视觉定位 + 分辨率标定**（默认，Flutter App 唯一路径）— 照截图看到的位置，按 `device_screenshot` 返回的 `scale`（或 §2 裸 adb 标定）换算成 tap 坐标。点不中先查 `scale`/标定比例对不对，**不是**「VLM 有偏差」
...
| 平台层 | argus.platforms.*（进程内） | argus MCP `device_*` tools（复用同一 platform，MCP 不可用时降级裸 adb） |
