...
def _should_skip_by_probes(case_text: str) -> str | None:
    ...
    """
    from .probes import has_probe_directive
...
def main():
    ...
    # argus probes <subcommand> — 非视觉断言插件（埋点/后端落库/上报日志）
    probes_p = sub.add_parser(
        "probes", help="Non-visual assertion plugins (analytics events, DB, logs)")
    probes_sub = probes_p.add_subparsers(dest="probes_command")
    ...
    pc.add_argument("name", help="Probe name as registered in .argus/probes.json")
    pc.add_argument("args", nargs="*",
                    help="k=v pairs, same syntax as the `# argus-probe:` directive")
    ...
    if args.command == "init":
        ...
        if not getattr(args, "probes_command", None):
            parser.parse_args(["probes", "--help"])
        else:
            cmd_probes(args)
...
def cmd_probes(args):
    """`argus probes list|check` —— 非视觉断言插件的发现 + 单发调试。
    ...
    from .probes import ProbeRunner, build_probe, summarize_data
    from .probes.spec import parse_directive
    ...
    if args.probes_command == "list":
        ...
        if not rows:
            print("  (空 —— 配一个 .argus/probes.json，可 cp .argus/probes.json.example)")
            return
        for r in rows:
    ...
    if spec is None:
        print("解析不出 probe 声明，检查参数语法（同 `# argus-probe:` directive）")
        raise SystemExit(2)
    ...
    while True:
        ...
        ctx = ProbeContext(
            step_index=0, step_text=f"[argus probes check] {spec.summary()}",
