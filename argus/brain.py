...
argus 是**纯视觉**决策：只看当前截图定位目标，没有 UI 元素树可依赖。
- **坐标一律用百分比**：`x_pct` = 目标中心在图片**宽度**的百分比(0=最左, 100=最右)，
  `y_pct` = 在**高度**的百分比(0=顶, 100=底)。**不要输出像素坐标**——你估百分比比估
  绝对像素准得多，且与设备分辨率无关。
- 点击时估目标的**几何中心**；对小元素（顶部图标、关闭按钮、右下发送键等）看清中心再给点。
- 测试用例里如有「Hints」或「位置参考」段（"右上角"、"X 按钮左边"），当辅助线索缩小范围。
...
def _pct_to_px(action: dict, w: int, h: int) -> None:
    """把 LLM 输出的百分比坐标(x_pct/y_pct, 0-100)换算成像素写回 x/y。

    实测 20+ 主流 VLM：问「百分比」定位精准且分辨率无关，问「绝对像素」会偏
    (同一模型同一按钮能差 ~7%)。故坐标协议改成 %，在此处统一换算成像素，
    下游(validator/executor/report)仍见像素 x/y，无需改动。
    兼容：模型若直接给了像素 x/y 则保留不覆盖；给 0-1 小数按比例处理。
    """
    if not isinstance(action, dict):
        return
    ...
    for px_key, py_key, dx, dy, dw, dh in (
        ("x_pct", "y_pct", "x", "y", w, h),
        ("x1_pct", "y1_pct", "x1", "y1", w, h),
        ("x2_pct", "y2_pct", "x2", "y2", w, h),
    ):
        if px_key in action and dx not in action:
            try:
                action[dx] = conv(action[px_key], dw)
            except (TypeError, ValueError):
                pass
        if py_key in action and dy not in action:
            try:
                action[dy] = conv(action[py_key], dh)
            except (TypeError, ValueError):
                pass
...
class Brain:
    ...
    def decide(self, test_case: str, screenshot_png: bytes,
        ...
        # Text payload — test case, skills text, full text history. 纯视觉：不喂 UI 树。
        user_content.append({
            "type": "text",
            "text": (
                f"{grid_section}"
        ...
        for attempt in range(1, LLM_MAX_RETRIES + 1):
            try:
                ...
                if isinstance(decision.get("action"), dict):
                    ...
                    _pct_to_px(decision["action"], w, h)
    ...
    def plan_step_actions(self, screenshot_png: bytes, step_text: str,
        ...
        prompt = (
            ...
            "⚠️ **弹窗优先**:若当前截图里有**拦截性弹窗 / 遮挡层**(系统权限请求、评分/更新提示、"
            "对话框等，按钮如「允许 / 不允许 / 好 / 关闭 / 知道了 / X」)挡住了要操作的界面，"
            "**先把它处理掉作为序列的第一个动作**(点对应按钮，权限类优先选「不允许 / 关闭」)，"
            "再接正常操作;这样即使执行时才冒出弹窗、重拆时也能先清掉它再继续。\n\n"
            f"操作步骤:\n{step_text}"
