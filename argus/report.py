...
def save_html(results: list[dict], output_path: str) -> str:
    ...
    for i, r in enumerate(results, 1):
        ...
        for s in r.get("steps_detail", []):
            ...
            # probe（非视觉断言插件）字段：verdict 来自代码层而非 LLM，是这类 step
            # 唯一的判定依据 —— 报告里必须能看到查了什么、查到什么、等了多久
            pr = s.get("probe") or {}
            probe_html = ""
            if pr:
                v = str(pr.get("verdict", "?"))
                v_color = {"pass": "#16a34a", "fail": "#dc2626",
                           "inconclusive": "#d97706",
                           "skipped": "#6b7280"}.get(v.lower(), "#666")
                probe_html = (
                    f'<div style="border-left:3px solid {v_color};'
                    f'margin:8px 0;background:#f5f3ff;border-radius:4px;'
                    f'padding:8px 12px;font-size:12px;">'
                    f'<div><b>probe:</b> <code>{_esc(pr.get("spec", pr.get("name", "")))}</code>'
                    f' → <code style="color:{v_color}">{_esc(v)}</code>'
                    f' <span style="color:#666">(第 {_esc(pr.get("attempt", 1))} 次查询，'
                    f'已等 {_esc(pr.get("elapsed_s", 0))}s / 预算 {_esc(pr.get("timeout_s", 0))}s)'
                    f'</span></div>'
                )
                if pr.get("evidence"):
                    probe_html += f'<div><b>evidence:</b> {_esc(pr["evidence"])}</div>'
                if pr.get("error"):
                    probe_html += (f'<div style="color:#b91c1c"><b>probe error:</b> '
                                   f'{_esc(pr["error"])}</div>')
                if pr.get("data"):
                    probe_html += (
                        f'<details><summary>原始数据</summary>'
                        f'<pre style="white-space:pre-wrap;word-break:break-all;'
                        f'max-height:320px;overflow:auto;">{_esc(pr["data"])}</pre>'
                        f'</details>')
                probe_html += '</div>'
