...
def _parse_elements_from_tree(ui_tree: str) -> list[dict]:
    """Extract element frames from the UI tree."""
    elements = []

    # Try JSON (iOS idb format)
    try:
    ...
    # Try XML (accessibility hierarchy)
    import xml.etree.ElementTree as ET
    try:
        root = ET.fromstring(ui_tree)
        for elem in root.iter():
            frame_str = elem.get("frame", "")
            m = re.match(
            ...
            if m:
                elements.append({
                    "type": elem.get("type", "") or elem.tag,
...
class LayoutCheckerSkill(Skill):
    ...
    def process(self, ctx: SkillContext) -> SkillResult:
        ...
        # Parse elements from UI tree
        elements = _parse_elements_from_tree(ctx.ui_tree)

        if elements:
            all_issues.extend(_check_overlaps(elements))
            all_issues.extend(_check_clipped(elements, w, h))
            all_issues.extend(_check_small_targets(elements))
            all_issues.extend(_check_alignment(elements))

        # Visual overflow detection (always works)
