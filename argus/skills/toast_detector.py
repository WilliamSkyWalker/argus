...
def _check_ui_tree_toast(ui_tree: str) -> list[str]:
    """Find toast-like elements in the UI tree."""
    hits = []
    lower = ui_tree.lower()
    for kw in _TOAST_KEYWORDS:
...
class ToastDetectorSkill(Skill):
    ...
    def process(self, ctx: SkillContext) -> SkillResult:
        ...
        # 1. UI tree check
        ui_hits = _check_ui_tree_toast(ctx.ui_tree)
        if ui_hits:
            results.append(f"UI 树中发现通知类元素: {', '.join(ui_hits)}")
            confidence += 0.6
