...
def _parse_ios_scroll_info(ui_tree: str) -> dict | None:
    """Extract scroll position from iOS UI tree JSON."""
    try:
    ...
    for node in nodes if isinstance(nodes, list) else [nodes]:
        ...
        if "ScrollView" in node_type or "TableView" in node_type or "CollectionView" in node_type:
            frame = node.get("frame", {})
            content_size = node.get("contentSize", {})
            content_offset = node.get("contentOffset", {})

            if content_size and frame:
                view_h = frame.get("height", 0)
                content_h = content_size.get("height", 0)
                offset_y = content_offset.get("y", 0) if content_offset else 0

                if content_h > view_h:
                    position = offset_y / (content_h - view_h) if content_h > view_h else 0
                    return {
                        "position": round(max(0, min(1, position)), 2),
                        "visible_fraction": round(view_h / content_h, 2) if content_h > 0 else 1.0,
...
class ScrollMapSkill(Skill):
    ...
    def process(self, ctx: SkillContext) -> SkillResult:
        ...
        # 1. iOS UI tree
        scroll_info = _parse_ios_scroll_info(ctx.ui_tree)

        # 2. Visual scrollbar detection
