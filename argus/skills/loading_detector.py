...
def _check_ui_tree(ui_tree: str) -> list[str]:
    """Scan UI tree for loading-related elements."""
    hits = []
    lower = ui_tree.lower()
    for kw in _LOADING_KEYWORDS:
...
class LoadingDetectorSkill(Skill):
    ...
    def process(self, ctx: SkillContext) -> SkillResult:
        ...
        # 1. UI tree keywords
        ui_hits = _check_ui_tree(ctx.ui_tree)
        if ui_hits:
            signals.append(f"UI 树中发现加载指示器: {', '.join(ui_hits)}")
            confidence += 0.5
