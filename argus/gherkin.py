...
- # argus-probe: <name> k=v —— step 级非视觉断言声明（埋点等，见 argus/probes/），
  绑定它上面最近的那个 step，随 case body 原样渲染下去给 agent 消费
...
# step 级 probe 声明（非视觉断言通道，见 argus/probes/）。**必须在 ARGUS_META
# 之前匹配** —— 否则会被当成 feature 级元数据吞掉（且多条互相覆盖）。
# 它绑定「上面最近的那个 step」，所以原样保留进 steps 列表随 case body 一起渲染，
# 由 agent 再按 step 顺序反查绑定关系（同 `- **Ref**:` 那套搭便车的做法）。
ARGUS_PROBE = re.compile(r'^\s*#\s*argus-probe:\s*(.+?)\s*$')
...
class Scenario:
    ...
    def platforms(self) -> list[str]:
        """该 scenario 声明的平台标签集合(可多个,可扩展 web/desktop 等)。
        每挂一个平台标签即在该平台跑；空 = 不限平台(哪个都跑)。
        """
        tags = set(self.tags)
        ps = [p for p in _KNOWN_PLATFORMS if p in tags]
        if 'both' in tags:  # 遗留写法 = android+ios
    ...
    def platform(self) -> str:
        """render/显示用：逗号连接平台集合；空则 'any'（不限）。"""
        ps = self.platforms
        return ", ".join(ps) if ps else "any"
...
def parse_feature(text: str, source_file: str = '') -> Feature:
    ...
    while i < len(lines):
        ...
        # step 级 probe 声明（挂在上一条 step 上，见 ARGUS_PROBE 注释）
        m = ARGUS_PROBE.match(line)
        if m:
            directive = f"# argus-probe: {m.group(1)}"
            if state == 'scenario' and current is not None and current.steps:
                current.steps.append(directive)
            elif state == 'background' and feature.background_steps:
                # Background 不进 step 主循环（render 成 `- **Background**:` 块），
                # 挂这儿的 probe 不会被执行 —— 保留原文但明确告警，别静默丢断言
                feature.background_steps.append(directive)
                log.warning("%s:%d Background 里的 argus-probe 不会执行"
                            "（Background 不进 step 循环），请挂到 Scenario 的 step 上",
                            source_file or '<inline>', i + 1)
            else:
                log.warning("%s:%d argus-probe 前面没有 step，已忽略: %s",
                            source_file or '<inline>', i + 1, directive)
            i += 1
            continue
