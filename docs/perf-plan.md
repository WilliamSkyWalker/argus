...
> + `step_validator.validate_assertion_batch`（逐条硬墙 + 去重闸 + 负向断言 absence 加压）+ 全 pass
> 一次推进多步。**有 fail 先探测弹窗**（`brain.dismiss_blocking_popup`）：合并是同步 inline、设备
> 还在那屏 → 有拦截性弹窗挡住就**直接关掉、重截图、重判**（最多 MAX_POPUP_DISMISS=2 次），避免
> "弹窗假 fail"；关掉后仍 fail 或压根没弹窗 → 真 fail。实机验证：base-001 一个 turn 判完、逐条
> 独立 evidence、step5 抓到真实 fail。离线验证：all-pass / 无弹窗真 fail / 弹窗→关→重判过 /
> 弹窗持续→2 次后判 fail 四态均正确。**默认开**。
> **注**：Phase 4（异步 Then）方案已否决（异步在 worker 末尾定案，设备已离屏，弹窗无法 live 关+重判）。
...
## Phase 4. Then 异步（已否决，不实施）

该方案原计划冻结断言帧后异步裁决，让设备立即进入下一个 case。最终否决原因是：连续
断言失败时框架需要在**当前现场**识别并关闭拦截性弹窗，再重新截图判定；异步任务返回时
设备通常已经离开该页面，无法完成 live 修复，会增加弹窗导致的假失败。

当前选择是保持 Phase 3 的同步同屏断言合并：一次调用减少延迟，同时保留现场关弹窗并重判
的能力。此 Phase 仅保留为决策记录，不在后续排期中。
...
Phase 4（Then 异步）──────────── 已否决，不实施
Phase 5（config 提速）──────────── 可与 1–3 并行验证
```

## 通用验证纪律
- 每个 Phase 落地后：同一基准 feature / 同 3 台 / 对照 baseline，看**墙钟 + 聚合 + 通过率**三项，任一回退即查。
- 通过率**不许因提速而下降**——尤其 Phase 3 的反偷懒与现场重判红线是硬约束，宁可不提速也不放松。
